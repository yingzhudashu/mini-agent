...
3. 配置事件订阅：
   ...
   - 订阅方式选择**长连接**（Receive events/callbacks through persistent connection）
   - ⚠️ **不需要配置请求地址**，长轮询模式无需公网 IP
4. 发布应用并获得相应权限
...
启动后，Agent 会自动通过 WebSocket 连接飞书服务器并接收消息。
...
- `src/feishu/poll-server.ts` — WebSocket 长轮询服务器
- `src/feishu/server.ts` — Webhook HTTP 服务器（备用，需要公网 IP 时使用）
